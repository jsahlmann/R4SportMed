...
ae <- sdtm_ae
dm <- sdtm_dm %>%
  select(USUBJID, ACTARMCD, ACTARM)
...
# n per Treatment als Einzelvariable
n_Pbo <- cnt_level_1 %>%
  filter(TRT == "Pbo") %>%
  select(cnt_1) %>%
  as.integer()
n_Xan_Hi <- cnt_level_1 %>%
  filter(TRT == "Xan_Hi") %>%
  select(cnt_1) %>%
  as.integer()
n_Xan_Lo <- cnt_level_1 %>%
  filter(TRT == "Xan_Lo") %>%
  select(cnt_1) %>%
  as.integer()

# Zeile 1 mit Any Adverse Events
cl1 <- cnt_level_1 %>%
  pivot_wider(names_from = TRT, values_from = cnt_1)
cl1$order <- 1
cl1$SOC <- "Any Adverse Event"
cl1$PT <- ""
cl1 <- cl1 %>%
  select(SOC, PT, Pbo, Xan_Hi, Xan_Lo)
...
# n per Treatment und System Organ Class
cl2 <- cnt_level_2 %>%
  pivot_wider(names_from = TRT, values_from = cnt_2)
cl2$order <- 2
cl2$SOC <- cl2$AEBODSYS
cl2$PT <- ""
cl2 <- cl2 %>%
  select(SOC, PT, Pbo, Xan_Hi, Xan_Lo, order)
cl2

# n per Treatment, System Organ Class und Preferred Term
cnt_level_3 <- df1 %>%
...
cl3 <- cnt_level_3 %>%
  pivot_wider(names_from = TRT, values_from = cnt_3)
cl3$order <- 3
...
ddf <- rbind(cl2, cl3) %>%
  arrange(SOC, PT)
df3 <- left_join(ddf, cl2 %>%
                   select(-c(PT)), by = c("SOC" = "SOC"))
...
ft_ae <- df4 %>%
  select(SOC_PT, Xan_Hi, Xan_Lo, Pbo) %>%
  flextable() %>%
  autofit()
ft_ae
